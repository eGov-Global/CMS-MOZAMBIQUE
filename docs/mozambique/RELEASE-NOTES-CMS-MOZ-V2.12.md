...
- **Admin console improvements** — role-actions editable from the UI, sensitive masters gated by role, translations propagate immediately on save, console boots in the environment's language, testing-tenant flag with guard rails. Build-time flags (`VITE_STATE_TENANT_ID`, telemetry kill switch, boundary/OSM endpoints) and every feature gate are documented in the [technical reference](https://github.com/eGov-Global/CMS-MOZAMBIQUE/blob/master/docs/mozambique/RELEASE-PREVIEW-CMS-MOZ-V2.12.md#the-admin-console-configurator--configuration-reference)
