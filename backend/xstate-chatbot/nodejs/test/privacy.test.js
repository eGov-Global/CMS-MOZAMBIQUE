...
test("a mobile number keeps only enough digits to correlate", () => {
  assert.equal(maskMobile("849904390"), "84*****81");
  assert.equal(maskMobile("258849904390"), "25********81");
  assert.equal(maskMobile("whatsapp:+258849904390"), "25********81", "non-digits are stripped first");
});
...
test("a webhook summary names the fields but reveals no content", () => {
  const summary = summarizeInbound({
    From: "whatsapp:+258849904390",
    To: "whatsapp:+258841817027",
  ...
  assert.ok(!summary.includes("849904390"), "the full number must not be logged");
